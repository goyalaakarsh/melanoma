...
def test_ph2_pipeline():
    ...
    try:
        ...
        # Validate feature extraction - FIXED KEY MISMATCHES
        ...
        feature_metrics = {
            ...
            'border_irregularity': features.get('border_irregularity', 0),
            'color_variation': features.get('color_variation', 0),
            'diameter_mm': features.get('largest_diameter_mm', 0),  # FIXED
            'texture_contrast': features.get('glcm_contrast', 0),  # FIXED
            'texture_homogeneity': features.get('glcm_homogeneity', 0)  # FIXED
        }
        ...
        # Step 8: Comprehensive Visualization Generation with Analysis Dashboard
        logger.log_step("VISUALIZATION", "INFO", "Creating comprehensive 3x3 analysis dashboard")
        ...
        color_viz = utils.visualize_color_clusters(hair_free_image, binary_mask, features.get('color_variation', 1))
        texture_viz = utils.visualize_texture(hair_free_image, binary_mask)

        # Create comprehensive 3x3 dashboard
        fig, axes = plt.subplots(3, 3, figsize=(24, 20))
        fig.suptitle(f'PH2 Dataset - Comprehensive Analysis Dashboard\nDice: {dice_score:.3f} | IoU: {iou:.3f} | F1: {f1_score:.3f} | Coverage: {our_coverage:.1f}%',
                     fontsize=18, fontweight='bold', y=0.995)

        # TOP ROW: Processing Pipeline
        # 1. Original Image
        axes[0, 0].imshow(rgb_image)
        axes[0, 0].set_title(f'1. Original PH2 Image\n{image_properties["shape"]}', fontweight='bold', fontsize=12)
        axes[0, 0].axis('off')

        # 2. Hair-free Image
        axes[0, 1].imshow(hair_free_image)
        axes[0, 1].set_title(f'2. Hair-Free Image\n{hair_removal_metrics["hair_percentage_removed"]:.1f}% hair removed', fontweight='bold', fontsize=12)
        axes[0, 1].axis('off')

        # 3. Final Segmentation Overlay
        overlay = utils.create_overlay_image(hair_free_image, binary_mask)
        axes[0, 2].imshow(overlay)
        axes[0, 2].set_title(f'3. Segmentation Overlay\nConfidence: {segmentation_metrics["confidence_score"]:.3f}', fontweight='bold', fontsize=12)
        axes[0, 2].axis('off')

        # MIDDLE ROW: Feature Analysis Visualizations
        # 4. Asymmetry Visualization
        axes[1, 0].imshow(asymmetry_viz)
        axes[1, 0].set_title(f'4. Asymmetry Analysis\nScore: {features.get("asymmetry_score", 0):.3f}', fontweight='bold', fontsize=12)
        axes[1, 0].axis('off')

        # 5. Border Visualization (with Convex Hull)
        axes[1, 1].imshow(border_viz)
        axes[1, 1].set_title(f'5. Border Irregularity\nScore: {features.get("border_irregularity", 0):.3f}', fontweight='bold', fontsize=12)
        axes[1, 1].axis('off')

        # 6. Color Cluster Map
        axes[1, 2].imshow(color_viz)
        axes[1, 2].set_title(f'6. Color Variation\n{features.get("color_variation", 0)} distinct colors', fontweight='bold', fontsize=12)
        axes[1, 2].axis('off')

        # BOTTOM ROW: Advanced Analysis
        # 7. Texture (LBP) Visualization
        axes[2, 0].imshow(texture_viz)
        axes[2, 0].set_title(f'7. Texture Analysis (LBP)\nContrast: {features.get("glcm_contrast", 0):.1f}', fontweight='bold', fontsize=12)
        axes[2, 0].axis('off')

        # 8. Color Hue Histogram
        hsv_lesion = hsv_image[binary_mask > 0]
        if len(hsv_lesion) > 0:
            hue_values = hsv_lesion[:, 0]
            axes[2, 1].hist(hue_values, bins=50, color='steelblue', alpha=0.7, edgecolor='black')
            axes[2, 1].set_title('8. Color (Hue) Distribution', fontweight='bold', fontsize=12)
            axes[2, 1].set_xlabel('Hue Value')
            axes[2, 1].set_ylabel('Frequency')
            axes[2, 1].grid(True, alpha=0.3)

        # 9. Risk Summary Text Box
        asymmetry_risk = min(1.0, features.get('asymmetry_score', 0) * 10)
        border_risk = min(1.0, (features.get('border_irregularity', 1) - 1.0) / 5.0)
        color_risk = min(1.0, (features.get('color_variation', 1) - 1) / 4.0)
        ...
        summary_text = f"""CLINICAL RISK SUMMARY

Overall Risk: {risk_level} ({overall_risk:.3f})

Individual Risk Scores:
• Asymmetry: {asymmetry_risk:.3f}
• Border: {border_risk:.3f}
• Color: {color_risk:.3f}
• Diameter: {diameter_risk:.3f} ({features.get('largest_diameter_mm', 0):.1f} mm)
• Texture: {texture_risk:.3f}
...
Not for medical diagnosis"""
        ...
        axes[2, 2].text(0.05, 0.95, summary_text, transform=axes[2, 2].transAxes, fontsize=10,
                        ...
                        bbox=dict(boxstyle='round', facecolor='lightyellow', alpha=0.8, edgecolor=risk_color, linewidth=3))
        axes[2, 2].axis('off')
        ...
        logger.log_step("VISUALIZATION", "SUCCESS", "Comprehensive 3x3 analysis dashboard saved as 'comprehensive_analysis_dashboard.png'")
